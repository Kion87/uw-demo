...
- ✅ Fixed-amount deposit invoices — an opt-in, per-user, all-assets setting (`User.fixedAmountInvoices`, toggled in the Deposits settings section) that lets a user request a deposit invoice for a specific amount instead of the always-reusable, no-amount address flow:
  ...
  - Deposit page shows the amount field (gated on the shared `useState("user")`, live-reactive to the Settings toggle) and a "Paid X of Y · Z due" breakdown in Recent Deposits; Dashboard pills show an amber "underpaid" state alongside the existing pending/complete/failed colors
...
- **Invoice callback payload shape is not yet confirmed against a real captured delivery.** Unlike `transaction_*`/`payout_*` (captured in `WEBHOOK_PAYLOADS.MD`), the `invoice_*` field mapping (`invoice.amount.requested`/`invoice.amount.paid`, the "three extra fields" Uniwire's docs mention without naming) is sourced from Uniwire's docs only. Code defensively as-is; capture a real example into `WEBHOOK_PAYLOADS.MD` once observed and reconcile if it differs.
- **A freshly-created fixed-amount invoice briefly shows raw status `"new"`** in Recent Deposits (and `"pending"` on the Dashboard) before the first `invoice_*` callback arrives — cosmetic, not a correctness issue, but slightly different from how reusable-address deposits only ever appear post-callback.
