...
- ✅ Payout callbacks (`payout_*`) handled in the same `POST /api/uniwire/callback` endpoint as deposit callbacks — matched by our own `referenceId` (falls back to `uniwirePayoutId` once known), status mapped to `initialized`/`confirmed`/`rejected`/`failed`
